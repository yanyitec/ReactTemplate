...
class PromiseA implements IPromise{
    ...
    constructor(executor:(resolve:(value?:any)=>void,reject:(reason?:any)=>void)=>void,sync?:boolean|string,useApply?:boolean|string){
        ...
        let promisePromise = promise.promise = function(target?:any):IPromise{
            ...
            target.then = (onfullfill,onreject)=>{promise.then(onfullfill,onreject);return target;};
            target.done = (onfullfill)=>{promise.done(onfullfill);return target;};
            target.fail = (onreject)=>{promise.then(null,onreject);return target;};
        ...
        function executeFullfill(status:PromiseStates,value:any,sync:boolean):PromiseA{
            ...
            let isSync = sync?(sync===true||sync==='sync'):promise.__promise_fullfill_sync;
            if(isSync) exec();else _async(exec,0);
            return promise;
        }
        if(executor){
            try{
    ...
    static resolve(value:any,sync?:boolean|string,useApply?:boolean|string):IPromise{
        let promise = new PromiseA((fullfill,reject)=>{
            fullfill(value);
        },sync,useApply);
